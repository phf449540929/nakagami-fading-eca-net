...
for i = -10:2:20
    ...
    snr = i;
    filename=sprintf('%s%d%s', './dataset/rayleigh/dataset-rayleigh-ldpc-', snr, 'db.csv');
    disp(filename)
    ...
    single_rayleigh_channel = comm.RayleighChannel(...
        'SampleRate',10000, ...
        'PathDelays',[0 1.5e-4], ...
...
%         rxd = r.*txSig;                       % Adding the fading
        ...
        [signal, pathGains] = step(single_rayleigh_channel, txSig);
        rxdn = awgn(signal, snr, 'measured');     % Noise and fading
